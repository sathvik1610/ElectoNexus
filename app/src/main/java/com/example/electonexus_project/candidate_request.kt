...
class candidate_request : ComponentActivity() {

    private var lastTextViewId: Int? = R.id.textViewcr
    ...
    private fun fbinitialise(){
        fbref.get().addOnSuccessListener { snapshot ->
            ...
            Toast.makeText(this,"In addValue ${error.message}",Toast.LENGTH_SHORT).show()
    ...
    private fun createView(voter : DataSnapshot){
        ...
        @SuppressLint("RestrictedApi")
        val containerLayout: ConstraintLayout = findViewById(R.id.containercandidaterequestlist)
    ...
    private fun delbutton(){
        val layout = findViewById<ViewGroup>(R.id.containercandidaterequestlist)
        for(i in layout.childCount -1 downTo  1){
            val child = layout.getChildAt(i)
            if(child.id != R.id.textViewcr){}
            layout.removeViewAt(i)
        }
        lastTextViewId = R.id.textViewcr
    }
